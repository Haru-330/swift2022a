...
var DATE = ""
var DEPARTURE_TEXT_FIELD = "出発地を入力" //保存用変数
var ARRIVAL_TEXT_FIELD = "到着地を入力" //保存用変数
...
class HomeViewController: UIViewController {
    ...
    override func viewDidLayoutSubviews(){
        ...
        departuremapImage.center = CGPoint(x: MapDeButtonWidth/2 + DeTextWidth, y: MapDeButtonY + MapDeButtonHeight/2)
        ...
        arrivalmapImage.center = CGPoint(x: MapArButtonWidth/2 + ArTextWidth, y: MapArButtonY + MapArButtonHeight/2)
        //日付のテキストフィールドの枠線を消す
    ...
    override func viewDidAppear(_ animated: Bool){
        ...
        if(departureButton.currentTitle != "出発地を入力"){
            departureButton.setTitleColor(UIColor.black, for: .normal)
        }
        if(arrivalButton.currentTitle != "到着地を入力"){
    ...
    override func viewDidLoad() {
        ...
        if(departureButton.currentTitle=="出発地を入力" || arrivalButton.currentTitle=="到着地を入力"){
    ...
    @objc func tapDameKensakuButton(_ sender: UIButton){
        alert(title: "エラー",
              message: "出発地バス停と到着地バス停を両方入力してください。")
    }
    ...
    @IBAction func tapMapButtonDep(_ sender: Any) {
        ...
        if DEPARTURE_TEXT_FIELD != "出発地を入力" {
            nextMapViewA.inDeparture = DEPARTURE_TEXT_FIELD
        }
        if ARRIVAL_TEXT_FIELD != "到着地を入力" {
